...
def BackwardEulerStep(T, y0, h):
    N = int(T/h)
    t0 = np.linspace(0, T, N+1)
    y = np.zeros(N+1)
    y[0] = y0
    # Implicit step: y_{n+1} = y_n + h * y_{n+1} * (1 - y_{n+1})
    # This is a quadratic in y_{n+1}: h y_{n+1}^2 + (1 - h) y_{n+1} - y_n = 0
    # Solve it explicitly and take the root in (0, 1).
    for i in range(N):
        a = h
        b = 1 - h
        c = -y[i]
        disc = b**2 - 4*a*c
        y[i+1] = (-b + np.sqrt(disc)) / (2*a)
    return y


# TESTING THE FUNCTION AND PLOTTING FOR DIFFERENT h (h ∈ [0.01,5]) and y(0) ∈ (0,1) (say, y(0) = 0.2))
if __name__ == "__main__":
    y0 = 0.2
    T = 10
    h_values = [0.01, 0.025, 0.1, 0.4, 1.6, 3.2, 5.0]
    ...
    plt.xlabel('t')
    plt.ylabel('y')
    plt.title("Backward Euler solutions for different step sizes, y(0) = 0.2, t ∈ [0,10]")
    plt.legend(loc='best')
    plt.grid(True)
    plt.tight_layout()
    plt.show()
    ...
    plt.title('Convergence of backward Euler (max error vs h)')
    plt.grid(True)
    plt.legend()
    plt.tight_layout()
    plt.show()
